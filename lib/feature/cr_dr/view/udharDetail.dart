...
import '../controller/udharDetailsController.dart';

class UdhaarDetailsScreen extends StatelessWidget {
  final controller = Get.put(UdhaarController());
  final RxBool isEditMode = false.obs;

  UdhaarDetailsScreen({super.key});
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                _statBox("Bill Amount", "₹${main.utinTotalAmt}", const Color(0xFF5A67D8)),
                const SizedBox(width: 10),
                _statBox("Outstanding", "₹${main.utinOutstandingAmt}", Colors.redAccent),
              ],
            ),
          ],
        ),
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Text("DEPOSIT HISTORY", style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.bold, color: tableHeaders)),
                    const Spacer(),
                    Obx(() => InkWell(
  ...
  Widget _buildBottomSummary(double total , BuildContext context, dynamic main) {
    final double cashBalance = double.tryParse(main.utinCashBalance?.toString() ?? '0') ?? 0;
    final String settleStatus = main.utinStatus;
    return Container(
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text("Total Deposited", style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey.shade700)),
  ...
  Widget buildFinalPaymentModes(UdhaarController controller) {
  ...
  Widget _compactDropdown(UdhaarController controller, {
